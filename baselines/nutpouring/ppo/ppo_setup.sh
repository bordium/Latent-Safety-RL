#!/usr/bin/env bash
# Install what the `ppo` extension needs on top of the shared Isaac Lab base
...
# Also installs skrl and the `ppo` extension itself.
#
...
ISAACLAB_TASKS_DIR="${ISAACLAB_TASKS_DIR:-$SCRIPT_DIR/IsaacLab}"
...
Usage: $SCRIPT_NAME COMMAND
  ...
  install-extension  pip install -e source/ppo into $CONDA_ENV
...
Useful overrides:
  ...
  ISAACLAB_TASKS_DIR=/abs/path    (default: ./IsaacLab, next to this script)
...
install_extension() {
  activate_conda
  require_file "$SCRIPT_DIR/source/ppo/setup.py"
  info "Installing the 'ppo' extension (editable) from $SCRIPT_DIR/source/ppo"
  python -m pip install -e "$SCRIPT_DIR/source/ppo"
}
...
PY
  ...
  # isaaclab_tasks / isaaclab_rl / ppo all transitively pull in `omni.*`,
  ...
  local pkg
  for pkg in isaaclab_tasks isaaclab_rl isaaclab_assets ppo; do
