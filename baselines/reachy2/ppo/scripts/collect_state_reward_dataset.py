...
pick_place_env_cfg.py) -- raw pixels are what you want for a data/correlation
study, since the policy's own normalization shifts frame to frame.

A flat .npy can't hold three differently-shaped arrays -- state is (N, 198),
vision is (N, H, W, 3), reward is (N, 9) -- so this saves a .npz bundling
them as named arrays instead: "state", "vision", "reward", "reward_term_names".
...
parser.add_argument(
    ...
    default=None,
    help=(
        "Output .npz path (default: baselines/ppo/results/<name>.npz, where <name> is the training"
        " run's timestamp -- e.g. checkpoints/.../2026-08-12_17-48-07_ppo_torch/checkpoints/agent.pt"
        " -> results/2026-08-12_17-48-07.npz -- or now() if run without --checkpoint)."
    ),
)
...
if args_cli.output is None:
    ...
    args_cli.output = os.path.normpath(os.path.join(results_dir, f"{output_name}.npz"))
