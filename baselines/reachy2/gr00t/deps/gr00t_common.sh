...
DATASETS_ROOT="${DATASETS_ROOT:-$ISAAC_ROOT/datasets/reachy2}"
DATASET_PATH="${DATASET_PATH:-$DATASETS_ROOT/Reachy2-Pick-Place-task}"
BASE_MODEL_PATH="${BASE_MODEL_PATH:-$ISAAC_ROOT/checkpoints/GR00T-N1-2B}"
TUNED_MODEL_PATH="${TUNED_MODEL_PATH:-$ISAAC_ROOT/checkpoints/GR00T-N1-2B-tuned-Reachy2-Pick-Place}"
RUN_DIR="${RUN_DIR:-$ISAAC_ROOT/training_runs/reachy2_pickplace_lora}"
RESULTS_DIR="${RESULTS_DIR:-$ISAAC_ROOT/eval_results/reachy2}"

BASE_MODEL_REPO="${BASE_MODEL_REPO:-nvidia/GR00T-N1-2B}"
TUNED_MODEL_REPO="${TUNED_MODEL_REPO:-}"
DATASET_REPO="${DATASET_REPO:-erl-hub/reachy-pick-and-place}"

TASK_NAME="${TASK_NAME:-reachy2pickplace}"

# Reachy2 is not one of GR00T N1's pretrained embodiments, so fine-tuning
# uses the generic "new_embodiment" tag (scripts/gr00t_finetune.py already
# defaults to it) plus a bespoke data config. The tag used at eval MUST
# match the one used at training -- gr00t/model/policy.py looks the
# normalization metadata up by tag and raises if it is absent.
EMBODIMENT_TAG="${EMBODIMENT_TAG:-new_embodiment}"
DATA_CONFIG="${DATA_CONFIG:-reachy2_bimanual}"
NUM_ENVS="${NUM_ENVS:-1}"
